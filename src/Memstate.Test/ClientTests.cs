...
namespace Memstate.Test
{
    public class ClientTests
    {
        ...
        public async Task HappyPath()
        {
            ...
            Assert.Equals(2, resultingVersion);

            //query
            var record = await client.Execute(new Get<int>("a"));
            Assert.Equals(2, record.Version);
            Assert.Equals(43, record.Value);
            ...
            Assert.Equals(record2.Value, record.Value);
            Assert.Equals(record2.Version, record.Version);
